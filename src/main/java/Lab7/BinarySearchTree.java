...
public class BinarySearchTree<T extends Comparable<T>>{
    ...
    public TNode<T> find(T data) { return find(data, root); }
    private TNode<T> find(T data, TNode<T> node) {
        if (node!= null) {
            int comp = node.getData().compareTo(data);
            if (comp == 0)
                return node;
            else if (comp > 0 && node.hasLeft()) return find(data, node.getLeft());
            else if (comp < 0 && node.hasRight()) return find(data, node.getRight());
        }
        return null;
    }
    ...
    public TNode<T> largest() { return largest(root); }
    ...
    public TNode<T> smallest() { return smallest(root); }
